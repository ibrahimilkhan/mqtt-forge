...
// so this launches a genuine second OS process (MqttForge.SingleInstanceProbe) instead
public sealed class SingleInstanceCrossProcessTests
{
    private static string UniqueName() => $"mqttforge-xproc-test-{Guid.NewGuid():N}";

    private static readonly string ProbePath = Path.Combine(AppContext.BaseDirectory,
        "MqttForge.SingleInstanceProbe.dll");

    [Fact]
    public async Task Second_process_is_refused_while_a_real_first_process_holds_the_lock()
    {
        Assert.True(File.Exists(ProbePath),
            $"probe not found at {ProbePath} - build MqttForge.SingleInstanceProbe first");
